...
\begin{abstract}
  \large We simulate the inspiral and binary formation of two supermassive black holes (SMBHs) within the bulge of an elliptical galaxy using the Barnes–Hut treecode algorithm. 
  The stellar background is modeled with three different spherically symmetric potential-density pairs: a regularized isothermal sphere (RIS), the King model, and the Hernquist model. 
  The SMBHs are initialized on wide circular orbits and evolve under the influence of dynamical friction. 
  Our simulations reproduce the expected inspiral phase, with the SMBHs sinking toward the galactic center and forming a bound binary at a separation consistent with the theoretical influence radius. 
  The measured dynamical friction accelerations show reasonable agreement with analytical estimates based on Chandrasekhar's formula applied to circular orbits in a singular isothermal sphere. 
  However, we find that the subsequent hardening of the binary is significantly slower than predicted by three-body interaction theory. 
  This discrepancy is attributed to the gravitational softening length being comparable to the binary separation, suppressing close stellar encounters. 
  We suggest future improvements using hybrid algorithms that combine direct integration for SMBH interactions with tree-based force evaluation for the rest of the system.
\end{abstract} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{equation}}
where $\ln \Lambda = \ln \left(b_\text{max}/b_\text{min}\right)$ is the Coulomb logarithm, $b$ is an impact parameter, $m$ is the mass of the background particles, $M$ is the mass of the perturber, and $f(v_m)$ is the distribution function of the background particles.
...
\begin{equation}
    ...
    &\approx 1 \left(\dfrac{M}{10^6 M_\odot}\right) \left(\dfrac{\sigma}{70 \text{km/s}}\right)^{-2} \text{pc} \approx \\
...
Moreover, once the MBHs are bound, dynamical friction acts on the center of mass of the binary, rather than on the individual MBHs.
...
More information on this process can be found in \cite{Quinlan1996}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{equation}
    p(v|r)dv = \dfrac{f(E(v|r))}{n(r)} 4\pi v^2 dv \:\: ,
...
\begin{equation}
    p_\text{SIS}(v|r)dv = \dfrac{4\pi}{\left(2\pi\sigma^2\right)^{3/2}} e^{-\frac{v^2}{2 \sigma^2}} v^2 dv \propto 
    \prod_{i=x,y,z} e^{-\frac{v_i^2}{2 \sigma^2}} v_i^2 dv_i \:\: .
    \label{eq:sis_distribution_function}
\end{equation}
We determine the value of $\sigma$ by imposing virial equilibrium:
...
\end{equation}}
and we sample the velocity components $v_i$ from Gaussians with mean $\mu=0$ and dispersion $\sigma$.
The resulting distribution is not exactly in hydrostatic equilibrium.
However, for our purposes, it suffices that the system remains close to virial equilibrium during the inspiral of the MBHs.
...
The relative potential is defined as $\psi(r) = \Phi(r_\text{truncation}) - \Phi(r)$, such that $\psi(r_\text{truncation}) = 0$.
...
\begin{equation}
    ...
    \label{eq:king_r0}
\end{equation}
is the core radius and $\rho_0 = \rho_1 \hat{\rho}(W_0)$ is the central density.
...
\end{equation}
This PDF can be integrated analytically to obtain the CDF, which can then be inverted to derive a sampling formula for the radial positions of the particles:
\begin{equation}
    r(P) = \dfrac{a}{P^{-1/2} - 1} \:\: ,
...
We are aware that this is a strong simplification, especially in the case of the King and Hernquist models, where the density and velocity profiles differ significantly from the SIS.
Nonetheless, it provides a starting point for comparing the theoretical predictions with the results of our simulations.
In this approximated case, the integrand simplifies (see Eq.~\ref{eq:velocity_pdf} and \ref{eq:sis_distribution_function}), and the integral becomes:
\begin{equation}
...
The Coulomb logarithm $\ln \Lambda$ typically ranges between 5 and 15, depending on the minimum and maximum impact parameters.
In our case, the predictions match the measured accelerations only when this value is set to $\ln \Lambda \approx 3$, which is significantly lower than the expected range; this discrepancy may be partially due to the softening radius $\epsilon$, which effectively reduces the minimum impact parameter of the interactions.
...
We have already estimated the velocity dispersion of the stellar background in Sec.~\ref{sec:analysis_dynamical_friction}.
...
The background stellar distribution was modeled using three different potential-density pairs: a regularized isothermal sphere (RIS), the King model, and the Hernquist model.
All three models reproduced the inspiral due to dynamical friction reasonably well, with the King and Hernquist models showing better stability in the Lagrangian radii.
The dynamical friction acceleration was computed from the simulation data and compared with theoretical predictions based on an approximation of Chandrasekhar's formula for the case of circular orbits in a singular isothermal sphere, which yielded decent agreement, even though the Coulomb logarithm had to be adjusted to match the measured accelerations.
The separation at which the binary formed was consistent with the theoretical estimate based on the influence radius.
...
Future work could focus on improving the modeling of stellar hardening by modifying the Barnes-Hut treecode to better resolve close encounters between stars and SMBHs.
In particular, it may be possible to compute the interactions involving the SMBHs using a direct N-body method, while continuing to use a tree algorithm for the rest of the system, thus avoiding a significant increase in computational cost.

Further improvements could be achieved by increasing the number of particles in the simulation, which would provide a more accurate representation of the stellar background and lead to a more realistic simulation of the binary hardening process.
Research-grade simulations of galactic bulges typically require $N \gtrsim 10^6$ particles, which is not feasible with the current implementation of the Barnes-Hut treecode running on a single CPU.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
