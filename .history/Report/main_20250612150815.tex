...
\begin{equation}
    p_\text{SIS}(v|r)dv = 4\pi \dfrac{n(r)}{\left(2\pi\sigma^2\right)^{3/2}} e^{-\frac{v^2}{2 \sigma^2}} v^2 dv \propto 
...
However, we can avoid this by introducing a simplified model that is commonly used for analytical calculations: we approximate the trajectories of the SMBHs as circular orbits within a singular isothermal sphere.
In this case, the circular velocity does not depend on the radius and is equal to $\sqrt{2} \sigma$, so the upper limit of the integral is constant.
