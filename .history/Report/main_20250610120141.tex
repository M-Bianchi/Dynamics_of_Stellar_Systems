...
For every spherically symmetric model, the angular probability distribution functions (PDFs) of the particles are given by:
\begin{equation}
    p(\phi) = \dfrac{1}{2 \pi} \:\: , \:\: p(\theta) = \dfrac{\sin \theta}{2}  \:\: ,
    \label{eq:angular_pdf}
\end{equation}
which can be easily integrated to obtain the cumulative distribution functions (CDFs).
The \textbf{direct Monte Carlo sampling method} allows to sample the angles $\phi$ and $\theta$ simply by generating a uniformly-distributed random variable $P \in [0,1]$ and inverting the CDFs:
\begin{equation}
...
Given the density profile, we can easily compute the radial probability distribution function (PDF) of the particles, which is given by:
\begin{equation}
