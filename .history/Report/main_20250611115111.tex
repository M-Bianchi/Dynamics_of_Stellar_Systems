...
Regarding the initial velocities, we can obtain their PDF $p_\text{King}(v|r)$  by plugging the distribution function of Eq. \ref{eq:king_distribution_function} into Eq. \ref{eq:velocity_pdf}.
...
For each particle at radius $r_i$, we can compute the escape velocity $v_\text{escape}(r_i)$ with the above equation, and then sample a proposal velocity $v_i \in [0, v_\text{escape}(r_i)]$.
If $p_\text{King}(v_i|r_i)$ 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
