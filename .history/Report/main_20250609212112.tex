...
\begin{itemize}[wide, labelwidth=!, itemindent=!, labelindent=0pt, itemsep=0.1em]
  \item \textbf{Integration time-step $dt$:} it is the time interval between two consecutive updates of the particles' positions and velocities. It must be small enough to ensure that the system evolves smoothly, but large enough to keep the computational time reasonable. 
  \item \textbf{Opening angle $\theta$:} it determines the accuracy of the treecode approximation, controlling how small the angle subtended by a cell can be before it is treated as a single particle.
  \item \textbf{Softening radius $\epsilon$:} the mass distribution is smoothed by replacing each particle by a Plummer sphere with scale length $\epsilon$, preventing the gravitational force from diverging at small distances. 
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
We chose $dt = 0.0005 \text{iu} \approx 7.5 \times 10^4 yr$. We chose $\theta=0.3$. We chose $\epsilon=0.1 \text{iu} = 100 pc$.
