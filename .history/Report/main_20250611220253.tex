...
\begin{equation}
    ...
    \end{split}
\end{equation}
where $q(r,v) \doteq \sqrt{\frac{E(r,v)}{\Phi(0)}}$ and $\Phi(r) = -\frac{GM_\text{tot}}{r+a}$.
...
The bulge has total mass of $M=10^{11} \,M_\odot$ and is modeled as a system of $N=50\,000$ particles of mass $m=2 \times 10^6 \,M_\odot$.
The length scale of the distribution is such that three fourths of the particles are contained within a sphere of radius $R_{75\%} = 5 \,\text{kpc}$.
This implies setting the scale length $R$ of the RIS model to $R \approx 3.7 \,\text{kpc}$ (and $r_\text{soft} \approx 1 \,\text{pc}$), the core radius $r_0$ of the King model to $r_0 \approx 155 \,\text{pc}$, and the scale length $a$ of the Hernquist model to $a \approx 800 \,\text{pc}$.
The resulting density and velocity profiles are shown in Fig. \ref{fig:profiles} for the three models.
The SMBHs are initialized on circular orbits at distance $R_{75\%}$ from the center of mass, one at position $\vec{r}_1 (t=0) = (R_{75\%}, 0, 0)$ with velocity $\vec{v}_1 (t=0) = (0, v_\text{circ}(R_{75\%}), 0)$, and the other at position $\vec{r}_2 (t=0) = (-R_{75\%}, 0, 0)$ with velocity $\vec{v}_2 (t=0) = (0, 0, v_\text{circ}(R_{75\%}))$.
The reason for this choice is minimizing the probability of a premature encounter between the two SMBHs, which would perturb the dynamical friction process.
\vspace{0.5em}
...
Higher values of $dt$ would lead to a significant loss of accuracy, undermining the stationarity of the Lagrangian radii shown in Fig. \ref{fig:Lagrangian_radii}.
In particular, the King and Hernquist models display remarkable stability over the full distribution, while the RIS model shows an expansion of the outer layers, which is expected due to the approximations we made.
In all three case, we can observe an initial readjustment of the Lagrangian radii — caused by the granularity of the distribution — followed by a second perturbation happening when the SMBHs move to the next shell.
