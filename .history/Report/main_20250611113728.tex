...
Given $\rho_\text{RIS}(r)$, one could derive the gravitational potential $\Phi_\text{RIS}(r)$ via the Poisson equation, and then obtain the distribution function $f_\text{RIS}(E(r,v))$ by solving the Eddington equation.
...
A more appropriate regularization of the SIS distribution is represented by \textbf{King models}, a class of spherically symmetric models developed by \cite{King1966}.
They are characterized by a stationary central density and a cutoff at large distances, parametrized by a truncation radius $r_\text{truncation}$.
They are defined by the following distribution function:
\begin{equation}
...
\end{equation}
where $\mathcal{E} = \psi(r) - \frac{1}{2} v^2$ is the relative energy, $\rho_1$ is a density scale-parameter, and $\sigma$ is a velocity scale-parameter.
The relative potential $\psi(r) = \Phi(r_\text{truncation}) - \Phi(r)$ is defined such that $\psi(r_\text{truncation}) = 0$.
The density profile of the King model as a function of $\psi$ is given by:
{\fontsize{8.5pt}{8.5pt}\begin{equation}
    \rho(\psi) = \rho_1 \left[ e^{\psi/\sigma^2} \text{erf}\left(\dfrac{\sqrt{\psi}}{\sigma}\right) - \sqrt{\dfrac{4 \psi}{\pi \sigma^2}} \left(1+\dfrac{2 \psi}{3 \sigma^2}\right) \right] = \rho_1 \hat{\rho}(\psi) \:\:,
    \label{eq:king_density}
\end{equation}}
where $\hat{\rho}(\psi)$ is the dimensionless density profile.
The potential profile $\psi(r)$ can be derived by solving the Poisson equation:
\begin{equation}
    \dfrac{1}{r^2} \dfrac{d}{dr} \left(r^2 \dfrac{d\psi}{dr}\right) = -4 \pi G \rho(r) \:\: .
    \label{eq:king_poisson}
\end{equation}
This can be expressed in terms of dimensionless quantities by defining the dimensionless potential $W(r) \doteq \psi(r)/\sigma^2$ and the dimensionless radius $x(r) \doteq r/r_0$, where:
\begin{equation}
     r_0 = \sqrt{\dfrac{9 \sigma^2}{4 \pi G \rho_0}} \:\:,
    \label{eq:king_r0}
\end{equation}
and $\rho_0 = \rho_1 \hat{\rho}(W_0)$ is the central density of the King model.
The resulting dimensionless Poisson equation is given by:
...
\end{equation}}
This second-order differential equation can be integrated numerically from $x=0$ to $x \rightarrow +\infty$.
The boundary conditions are $W'(x=0)=0$ and $W(x=0)=W_0$, where a low $W_0$ ($\approx 3$) corresponds to a diffuse distribution, while a large $W_0$ ($\approx 12$) implies a highly concentrated profile. $W_0 = 9$ is commonly considered an appropriate value for modeling galactic bulges.
Once we have obtained the dimensionless potential profile $W(x)$, we can easily compute the dimensionless density profile $\hat{\rho}(x)$ using Eq. \ref{eq:king_density}.
To obtain the actual density profile $\rho_\text{King}(r)$, we have to choose a scale for $r_0$ and $\rho_0$, which will be discussed in the next section.
The last step is plugging $\rho_\text{King}(r)$ into the Eq. \ref{eq:ris_pdf} to obtain the radial PDF of the particles, which can be sampled using the acceptance-rejection method.
\vspace{0.5em}

Regarding the initial velocities, we can sample them by using the distribution function of Eq. \ref{eq:king_distribution_function} with Eq. \ref{eq:velocity_pdf}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
