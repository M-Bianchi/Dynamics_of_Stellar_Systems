...
\end{equation}
where $n(r)$ is the number density of particles at radius $r$ and $f(\mathcal{E}(r,v))$ is the distribution function, describing the number of the particles per unit of phase-space volume.
...
Nonetheless, this is a complex task that goes beyond the scope of this naive model.
Therefore, we approximate the distribution function of the RIS model with that of the SIS model, which is a three-dimensional Maxwellian distribution with dispersion $\sigma$, where each component is independent and identically distributed:
\begin{equation}
...
\end{equation}
We impose the virial theorem to determine the value of $\sigma$:
...
\end{equation}}
The resulting distribution is not strictly in hydrostatic equilibrium.
However, for our purposes it is sufficient that the system does not deviate too much from the virial equilibrium, which can be achieved by choosing a sufficiently large number of particles and a small enough integration time-step.
