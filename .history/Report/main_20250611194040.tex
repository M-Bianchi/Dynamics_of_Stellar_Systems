...
The conversion factors between physical units and the internal units (iu) employed by the Barnes-Hut treecode algorithm are $1 \,\text{iu} \approx 10^7 M_\odot \approx 149.1 \,\text{Myr} \approx 5 \text{kpc}$.
To achieve an accurate simulation of the system, we evolve it from $t=0$ to $t=15 \,\text{iu} \approx 2.236 \,\text{Gyr}$, using an integration time-step of $dt = 0.0005 \,\text{iu} \approx 7.5 \times 10^4 \,\text{yr}$, an opening angle of $\theta=0.3$, and a softening radius of $\epsilon=0.1 \,\text{iu} = 100 \,\text{pc}$.
