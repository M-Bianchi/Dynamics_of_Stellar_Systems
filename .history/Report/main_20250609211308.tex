...
\begin{itemize}[wide, labelwidth=!, itemindent=!, labelindent=0pt, itemsep=0.1em]
  \item \textbf{Integration time-step $dt$:} it is the time interval between two consecutive updates of the particles' positions and velocities. It must be small enough to ensure that the system evolves smoothly, but large enough to keep the computational time reasonable. We chose $dt = 0.0005$.
  \item \textbf{Opening angle $\theta$:} it determines the accuracy of the treecode approximation, controlling how small the angle subtended by a cell can be before it is treated as a single particle. We chose $\theta=0.3$.
  \item \textbf{Softening radius $\epsilon$:} the mass distribution is smoothed by replacing each particle by a Plummer sphere with scale length $\epsilon$, preventing the gravitational force from diverging at small distances. We chose $\epsilon=0.1$.
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
