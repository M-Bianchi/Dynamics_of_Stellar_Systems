...
In Sec.~\ref{sec:introduction_dynamical_friction} we introduced the concept of dynamical friction, which is the process that causes a massive object moving through a stellar system to lose energy and angular momentum, thus spiraling inward.
We can observe this effect in the trajectories of the two SMBHs shown in Fig. \ref{fig:Lagrangian_radii}: while the rest of the distribution remains approximately stationary, the two SMBHs sink towards the center of mass of the system, crossing a radial distance of $\Delta r \approx 5 \,\text{kpc}$ in $\Delta t \approx 1 \,\text{Gyr}$.
Once the first SMBH reaches the dense core of the system, it begins orbiting around it at a distance of a few hundreds of parsecs from the center of mass.
When the second SMBH moves close enough, the two form a bound state, and the dynamical friction process becomes inefficient.
\vspace{0.5em}

To quantitatively compare the efficiency of dynamical friction in our simulations with the theoretical predictions, we compute the acceleration of the SMBHs with finite differences:
\begin{equation}
    \vec{a}_i(t) \simeq \dfrac{\vec{v}_i(t+dt) - \vec{v}_i(t)}{dt} \:\: .
    \label{eq:acceleration}
\end{equation}
This includes the centripetal acceleration due to the Keplerian motion, which must be subtracted to isolate the dynamical friction contribution:
\begin{equation}
...
The theoretical dynamical friction acceleration is given by Eq. \ref{eq:chandrasekhar}, which includes the integral — from $v_m=0$ to $v_m=v_M$ — of the distribution function of the background particles.
This equation holds under the assumption that the background is homogeneous and isotropic, which is not the case in our simulations, since the mass density profile varies by several orders of magnitude as we move towards the center.
This implies that the distribution function depends on the radial position, and therefore the integral in Eq. \ref{eq:chandrasekhar} must be computed at each time-step.
However, we can avoid this by introducing a simplified model that is commonly used for analytical calculations: we approximate the trajectories of the SMBHs as circular orbits within a singular isothermal sphere (SIS).
The approximated integrand is given by Eq. \ref{eq:sis_distribution_function}, whose integral becomes:
\begin{equation}
    I = 4 \pi \int_0^{v_M} f(v_m) v_m^2 dv_m = n(r) g(v_M) \:\:,
    \label{eq:chandrasekhar_sis_integral}
\end{equation}
where $g(v_M)$ is given by the integral of the Maxwellian distribution function:
\begin{equation}
...
\end{equation}
In the SIS model, the circular velocity does not depend on the radius and is equal to $\sqrt{2} \sigma$, so we obtain $g(v_M) \approx 0.428$.
Finally, we can write the approximated form of Eq. \ref{eq:chandrasekhar}:
\begin{equation}
    \dfrac{d\vec{v}_M}{dt} \, \simeq \, -0.428 \times 2 \pi G^2 n(r) m (m+M) \ln \Lambda \dfrac{\hat{v}_M}{\sigma^2} \:\: .
    \label{eq:chandrasekhar_sis}
\end{equation}
To adapt this approximated equation to our models, we replace the density and the velocity dispersion with estimates at the position of the SMBH, i.e., $n(r_i(t))$ and $\sigma(r_i(t))$.
In particular, we compute these quantities by considering only the tenth of particles closer to the SMBH at each time.
The Coulomb logarithm commonly assumes values between 5 and 15, depending on the minimum and maximum impact parameters of the interaction.
In our case, we set $\ln \Lambda = 3$ to match the amplitude of the accelerations computed from the data.
The expected dynamical friction acceleration is in decent agreement with the net acceleration of the SMBHs computed from the data, even though the former has been computed using an approximated model.
The results for one SMBH are shown in figure \ref{fig:Acceleration}.
The deviation of the data from the theoretical prediction increases as the SMBH approaches the center of mass, especially for the King model. 
The narrow peaks in the acceleration computed from the data correspond to premature encounters between the two SMBHs, which cause a sudden change in the velocity.
...
\begin{figure}\centering
	\includegraphics[width=1.\columnwidth]{LR_50K_dt0005_titles.png}
