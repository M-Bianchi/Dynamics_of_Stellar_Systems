...
In particular, we compute these quantities using the nearest 10\% of the particles to the each SMBH at a given time.
...
\begin{figure}\centering
    ...
    \caption{Time evolution of the SMBHs separation (red) for the RIS model (upper panel), the King model (middle panel), and the Hernquist model (lower panel). The mean influence radiaus (green) is also shown. The dashed lines indicate the time and the separation at which the binary forms.}
