...
Once the binary forms, the separation continues to decrease, as shown in Fig. \ref{fig:Separation}.
...
This process is characterized by a gradual decrease in the binary separation, which can be approximated by a power law:
\begin{equation}
    \dfrac{da}{dt} \, = \, - H \dfrac{G \rho(r_\text{influence}) a^2}{\sigma(r_\text{influence})} \:\: ,
    \label{eq:hardening}
\end{equation}
where $H$ is a dimensionless parameter that typically ranges between $5$ and $20$.
However, our attempts to model this process failed to reproduce the data, since we obtained a tiny hardening coefficient of $H_\text{measured} \approx 0.05$, which is significantly lower than the expected values.
This discrepancy is probably due to the fact that our simulation uses a softening radius $\epsilon = 100 \,\text{pc}$ that is of the same order of the binary separation.
Stellar hardening is a three-body process that requires the presence of stars in the vicinity of the binary, and a large softening radius prevents the simulation from resolving these interactions.
On the other hand, attempts to reduce the softening radius to $10 \,\text{pc}$ lead to numerical instabilities as soon as the SMBHs reach the central region, with the particles being ejected from the system at high velocities.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
