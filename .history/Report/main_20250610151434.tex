...
In general, this should be achieved by sampling from the conditional velocity PDF of the model, $p(v|r)$.
This, in turn, is obtained by integrating the distribution function $f(\mathcal{E}(r,v))$ — which defines the number of particles per unit of phase-space volume — over $dv^3$, while keeping the position $r$ fixed.
Finally, $f(\mathcal{E}(r,v))$ can be derived from the density profile $\rho(r)$ by assuming that the system is in \textbf{thermal equilibrium}, i.e., that the velocity distribution is isotropic and follows a Maxwell-Boltzmann distribution.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
