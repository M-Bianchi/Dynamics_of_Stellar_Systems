...
\end{equation}
where $n(r)$ is the number density at radius $r$, and $f(\mathcal{E}(r,v))$ is the distribution function describing the number of the particles per unit of phase-space volume.
Given $\rho(r)$, one could derive the gravitational potential $\phi(r)$ via the Poisson equation, and then obtain the distribution function $f(\mathcal{E}(r,v))$ by solving the Eddington equation.
